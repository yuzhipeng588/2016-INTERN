...
/*=============================================================================
   ...
   file is saved.
        ...
        Filename:  createbox3_template.c
...
{
 {UF_STYLER_DIALOG_INDEX, UF_STYLER_OK_CB           , 0, BLOCK_block_ok_cb},
 {UF_STYLER_DIALOG_INDEX, UF_STYLER_APPLY_CB        , 0, BLOCK_block_apply_cb},
 {UF_STYLER_DIALOG_INDEX, UF_STYLER_CANCEL_CB       , 0, BLOCK_block_cancel_cb},
 {BLOCK_POINT           , UF_STYLER_ACTIVATE_CB     , 1, BLOCK_point_act_cb},
...
static UF_MB_styler_actions_t actions[] = {
    { "createbox3.dlg",  NULL,   BLOCK_cbs,  UF_MB_STYLER_IS_NOT_TOP },
...
1)  Add the following lines to your MenuScript file in order to 
    ...
    The button, CREATEBOX3_BTN is set up to launch your dialog and 
...
!   ----------------------------------------------------------------
    ...
    MENU UISTYLER_DLG_CASCADE_BTN
      BUTTON CREATEBOX3_BTN
      LABEL Display createbox3 dialog
      ACTIONS createbox3.dlg
    END_OF_MENU
!   ---------------------------------------------------------------
...
4) You should also add the funcitonal prototype to your header file 
   (createbox3.h) and ensure that the file is properly included.
...
extern int <enter the name of your function> ( int *response )
{
    ...
    if ( ( error_code = UF_STYLER_create_dialog ( "createbox3.dlg",
...
--------------------------------------------------------------*/
static void get_real_data( int dialog_id,char *item_id,double *value)
{
...
static void create_block2( int dialog_id,double corner_pt[3])
{
	...
	/*get_real_data( dialog_id, my_real_3, &corner_pt[0]);
	get_real_data( dialog_id, my_real_4, &corner_pt[1]);
	get_real_data( dialog_id, my_real_5, &corner_pt[2]);*/
	...
	/*edge_len[3] = (char *)UF_allocate_memory( 256 * sizeof(char), &error );
	edge_len[4] = (char *)UF_allocate_memory( 256 * sizeof(char), &error );
	edge_len[5] = (char *)UF_allocate_memory( 256 * sizeof(char), &error );*/

	sprintf( edge_len[0], "%f", length );
	sprintf( edge_len[1], "%f", width );
	sprintf( edge_len[2], "%f", height );
	/*sprintf( edge_len[3], "%f", corner_pt[0]);
	sprintf( edge_len[4], "%f", corner_pt[1]);
	sprintf( edge_len[5], "%f", corner_pt[2]);*/
...
/* -------------------------------------------------------------------------
 * Callback Name: BLOCK_block_cancel_cb
...
/* -------------------------------------------------------------------------
 * Callback Name: BLOCK_block_apply_cb
 * This is a callback function associated with an action taken from a
 * UIStyler object. 
 *
 * Input: dialog_id   -   The dialog id indicate which dialog this callback
	...
	UF_STYLER_item_value_type_p_t callback_data)
{
	...
	/* ---- Enter your callback code here ----- */
	create_block2(dialog_id,corner_pt);
	UF_terminate ();
...
/* -------------------------------------------------------------------------
 * Callback Name: BLOCK_point_act_cb
 * This is a callback function associated with an action taken from a
 * UIStyler object. 
 *
 * Input: dialog_id   -   The dialog id indicate which dialog this callback
